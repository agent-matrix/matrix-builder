...
class BundleVersion(Base):
    ...
    parent_version_id: Mapped[str | None] = mapped_column(String(36), ForeignKey("bundle_versions.id"))
...
class BuildBatch(Base):
    ...
    id: Mapped[str] = _pk()
    owner_id: Mapped[str] = mapped_column(String(36), nullable=False, index=True)
    version_id: Mapped[str] = mapped_column(String(36), ForeignKey("bundle_versions.id", ondelete="CASCADE"), nullable=False, index=True)
    ...
    change_type: Mapped[str] = mapped_column(String(32), nullable=False)  # small-update|add-feature|fix-issue
...
class MatrixCommit(Base):
    ...
    id: Mapped[str] = _pk()
    owner_id: Mapped[str] = mapped_column(String(36), nullable=False, index=True)
    batch_id: Mapped[str] = mapped_column(String(36), ForeignKey("build_batches.id", ondelete="CASCADE"), nullable=False, index=True)
    version_id: Mapped[str] = mapped_column(String(36), ForeignKey("bundle_versions.id", ondelete="CASCADE"), nullable=False, index=True)
    parent_commit_id: Mapped[str | None] = mapped_column(String(36), ForeignKey("matrix_commits.id"))
...
class PromptVersion(Base):
    ...
    constraints: Mapped[dict] = mapped_column("constraints_jsonb", JSONB, default=dict, nullable=False)
...
class ValidationRun(Base):
    ...
    commit_id: Mapped[str | None] = mapped_column(String(36), ForeignKey("matrix_commits.id", ondelete="CASCADE"), index=True)
    batch_id: Mapped[str | None] = mapped_column(String(36), ForeignKey("build_batches.id", ondelete="CASCADE"), index=True)
    status: Mapped[str] = mapped_column(String(32), nullable=False)  # running|approved|needs-repair|rejected|failed
...
class ValidationFinding(Base):
    ...
    id: Mapped[str] = _pk()
    owner_id: Mapped[str] = mapped_column(String(36), nullable=False, index=True)
    validation_run_id: Mapped[str] = mapped_column(String(36), ForeignKey("validation_runs.id", ondelete="CASCADE"), nullable=False, index=True)
...
class RunEvent(Base):
    ...
    id: Mapped[str] = _pk()
    owner_id: Mapped[str] = mapped_column(String(36), nullable=False, index=True)
    run_id: Mapped[str] = mapped_column(String(36), ForeignKey("validation_runs.id", ondelete="CASCADE"), nullable=False, index=True)
...
class Artifact(Base):
    ...
    owner_id: Mapped[str] = mapped_column(String(36), nullable=False, index=True)
    project_id: Mapped[str] = mapped_column(String(36), ForeignKey("projects.id", ondelete="CASCADE"), nullable=False, index=True)
    version_id: Mapped[str | None] = mapped_column(String(36), ForeignKey("bundle_versions.id", ondelete="CASCADE"))
    commit_id: Mapped[str | None] = mapped_column(String(36), ForeignKey("matrix_commits.id", ondelete="CASCADE"))
    artifact_type: Mapped[str] = mapped_column(String(32), nullable=False)  # bundle_zip|prompt_md|patch_diff|log|manifest|thumbnail_svg
